...
class Audit {
    ...
    async startPolling() {
        ...
        while (true) {
            const accounts = await this._transactions.asyncGetAll();
            for (let i = 0; i < accounts.length; i++) {
                const account = accounts[i];
                for (let j = 0; j < account.transactions.length;) {
                    let error, stat;
                    const tx = account.transactions[j];
                    [error, stat] = await nothrow(this.asyncCheckReceipt(tx.txid, tx.date));
                    if (error != null) {
                    ...
                    if (stat == STATWAIT) {
                        ...
                        await nothrow(this._nonce.asyncSetInvalid(account.account, tx.nonce));
                        logger.error('[audit] Audit failure, account: %s, txid: %s, nonce: %s', account.account, tx.txid, tx.nonce);
                        break;
                    }
                    await nothrow(this._transactions.asyncDeleteTx(account.account, tx.nonce));
    ...
    // 检查凭据
    async asyncCheckReceipt(txid, createdAt) {
        let error, tx;
